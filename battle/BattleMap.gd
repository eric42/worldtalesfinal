...
# Inimigos que podem ser atacados.
var attack_tiles: Array[Vector2i] = []

# Preview do caminho de movimento.
var preview_path: Array[Vector2i] = []

# Tiles destacados quando o mouse passa sobre um alvo.
...
func show_reachable_tiles(tiles: Array[Vector2i]) -> void:
	...
	reachable_tiles = tiles.duplicate()
...
func show_attack_tiles(tiles: Array[Vector2i]) -> void:
	...
	attack_tiles = tiles.duplicate()

	# Quando uma nova lista de ataques é criada,
	# o hover anterior não deve permanecer.
	hover_attack_tiles.clear()
...
func show_hover_attack_tiles(tiles: Array[Vector2i]) -> void:
	...
	hover_attack_tiles = tiles.duplicate()
...
func _setup_astar() -> void:
	...
	astar.default_compute_heuristic = (
		AStarGrid2D.HEURISTIC_MANHATTAN
	)

	astar.default_estimate_heuristic = (
		AStarGrid2D.HEURISTIC_MANHATTAN
	)
...
func compute_attack_tiles(
	...
	for enemy in units_container.get_children():
		...
		for pos in possible_positions:
			...
			if (
				dist >= unit.attack_range_min
				and dist <= unit.attack_range_max
			):
...
func compute_attack_range_area(
	unit: HeroUnit,
	reachable_tiles: Array[Vector2i]
) -> Array[Vector2i]:
	...
	for pos in positions:
		...
		for x in range(
			-unit.attack_range_max,
			unit.attack_range_max + 1
		):
...
func get_attackable_units(
	...
	for tile in tiles:
		...
		var target: HeroUnit = get_unit_at(
			tile
		)
